...
export function ParticleSystem({ count, radius, radiusVariability, color }: ParticleSystemProps) {
  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const [x, y, z] = randomPointOnSphere(radius, radius * radiusVariability);
      positions[i * 3] = x;
      positions[i * 3 + 1] = y;
      positions[i * 3 + 2] = z;
    }
    return positions;
  }, [count, radius, radiusVariability]);
